...
public static class MaintenanceRecordDataFaker
{
    ...
    public static Faker<MaintenanceRecordResponse> MaintenanceRecordResponseFaker =>
        new Faker<MaintenanceRecordResponse>()
            .CustomInstantiator(
                f => new MaintenanceRecordResponse(
                    Guid.NewGuid(),
                    f.PickRandom(Types),
            ...
            .RuleFor(
                m => m.EndDate,
                (f, m) => f.Random.Bool() ? m.StartDate.AddDays(f.Random.Int(1, 30)) : null);
    ...
    public static Faker<MaintenanceRecordRequest> MaintenanceRecordRequestFaker => new Faker<MaintenanceRecordRequest>()
        ...
        .RuleFor(
            m => m.EndDate,
            (f, m) => f.Random.Bool() ? DateOnly.Parse(m.StartDate).AddDays(f.Random.Int(1, 30)).ToString() : null);
}
